...
var gP,
 ...
 window.dongyi=window.castapp = window.ca =  {
	 ...
	 * MM-dd 获取当前月份-日期
	 */
	getCurrentTime8Format:function(fmt){
		if(fmt==null || fmt==""){
			fmt = new Date();
		}else{
			...
			var o = {
			        ...
			        "W"  : weekday[currentDate.getDay()]			 //星期
			};
			if(/(y+)/.test(fmt)) {
			        fmt=fmt.replace(RegExp.$1, currentDate.getFullYear()); 
			}
			for(var k in o) {
...
//			jsonData.showType = 'zoom-fade-out';
		...
		if(isEmpty(jsonData.extras)) {
			jsonData.extras=null;
		}
	...
	ajax:function(json){
		...
		json.time=json.time ||  20;
		json.data=json.data || {};
		json.headerkeys=json.headerkeys || null;
		json.headervals=json.headervals || null;
		json.headerData=json.headerData || null;
		json.isMask=(json.isMask == undefined)?true:json.isMask;
		json.data=json.data || {};
		if(true==json.isMask) {
			dongyi.showWaiting();
		}
		...
		switch(json.type.toLowerCase()){
			case 'get':
				...
				if(!isEmpty(json.headerData)) {
						oAjax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
					for( filed in json.headerData) {
						//alert(filed+":"+);
						oAjax.setRequestHeader(filed,json.headerData[filed]);
					}
					
				}
				/*if(!isEmpty(json.headerkeys) && !isEmpty(json.headervals)) {
					for(var i=0;i<json.headerkeys.split(',').length;i++){
						//console.log(json.headerkeys.split(',')[i]+":"+ json.headervals.split(',')[i]);
						oAjax.setRequestHeader(json.headerkeys.split(',')[i],json.headervals.split(',')[i]);
					}
				}*/
			...
			case 'post':
				...
				oAjax.setRequestHeader('Content-Type','application/x-www-form-urlencoded;charset=utf-8');
		 		if(!isEmpty(json.headerData)) {
					for( filed in json.headerData) {
						// console.log(filed+":"+json.headerData[filed]);
						oAjax.setRequestHeader(filed,json.headerData[filed]);
					}
				}
			...
			/*	if(!isEmpty(json.headerkeys) && !isEmpty(json.headervals)) {
				...
				console.log(json.url+'param:'+json2url(json.data));
			...
			dongyi.closeWaiting();
			console.log(e);
			console.log(json2Str(e));

			//dongyi.prompt('请求异常:'+e);
			dongyi.prompt('请求异常:'+e.message);
			throw e;
		}
		oAjax.onreadystatechange=function(){
			...
			//	alert("onreadystatechange:" +oAjax.status );
				if(oAjax.status==0) {
					clearTimeout(timer);
					castapp.prompt('服务连接失败！');
				}if(oAjax.status==801) {
					castapp.prompt('登录失效！');
					clearTimeout(timer);
					castapp.toLoginPage();
					//json.errFn && json.errFn(oAjax.status);
				}else if(oAjax.status>=200 && oAjax.status<300 || oAjax.status==304){
					clearTimeout(timer);
	...
	toLoginPage:function (){
		...
		console.log("currentId:"+currentId +":" +currentId.toString().indexOf("login"));
		if(currentId.toString().indexOf("login")<0) {
